    ...
    updateMessage: function(key) {
        ...
        self.usermappingRef.child(ServeruserId).once('value', function(snap) {
            ...
            if (snap.child(self.path.usermapping_FBuser_field).exists()) {
                ...
                UserMessageRef = self.userRootRef.child(FBuserId).child(self.path.user_message_field).child(self.event);
                    ...
                    snap.forEach(function(childsnap) {
                        ...
                        childsnap.child('PO_HEADERS')
                            .forEach(function(POHeader_childsnap) {
                                ...
                                //in users/simplelogin%3A25/messageMetadata/E0001/02_PU/4500017496 insert messageMetadata
                                var Metadata_ref = self.userRootRef.child(FBuserId).child(self.path.user_messageMetadata_field)
                                    .child(self.event).child(POId);
                                Metadata_ref.transaction(function(currentData) {
                                    if (currentData === null) {
                                        return metadata;
                                    }
                                    else {
                                ...
                                var HeadData = {
                                    ...
                                    "data": {
                                        "date": POHeader_childsnap.child('doc_DATE').val().valueOf(),
                                        "favorite": false,
                                ...
                                var po_ref = self.userRootRef.child(FBuserId).child(self.path.user_message_field)
                                    .child(self.event).child(POId);
