                ...
                snap.forEach(function(childsnap) {
                    ...
                    childsnap.child('PO_HEADERS')
                        .forEach(function(POHeader_childsnap) {
                            ...
                            var po_ref=self.userRef.child(FBuserId).child(self.path.user_message_field)
                                                    .child(self.event);
                                                    
                                po_ref.child(POId).transaction(function(currentData) {
                                if (currentData === null) {
                                    return messageData;
                                }
                                else {
                            ...
                            var messageCount;
                            po_ref.once('value', function(snap) {
                                messageCount=snap.numChildren();
                                console.log(po_ref.toString(),messageCount);
                            });
                            var component_data={
                                 "config": 'x',
                                 "last": today.valueOf(),
                            ...
                            //users/simplelogin%3A25/components 插入message数据
                            self.userRef.child(FBuserId).child(self.path.user_component_field)
                                                    .child(self.event)
                                ...
                                if (error) {
                                    ...
                                    console.log('We aborted the transaction (because ' + FBuserId + '/' + self.event + '/' + POId + ' already exists).');
                                }
                                else {
                                    console.log(FBuserId + '/' + self.event + ' added!'.green);
                                }
                                // console.log("data: ", snapshot.val());
                            });
