...
function PathMonitor(firebaseUrl, path) {
    this.ref = fbutil.fbRef(firebaseUrl, path.path); //root of E0001 events
...
PathMonitor.prototype = {
    ...
    updateMessage: function(key) {
        var self = this;
        var userId = key;
        var UserMessageRef = self.userRef.child(userId).child('messages/E0001');
        var rootRefStr = self.ref.root().toString();

        //将 Event/E0001/100001/02_PU/PO_HEADERS/4500017496
        //更新到user/100001/messages/E0001/02_PU/4500017496

        this.ref.child(userId).once('value', function(snap) {
                    ...
                    .forEach(function(POHeader_childsnap) {
                        ...
                        User_PORef.transaction(function(currentData) {
                            ...
                            if (error) {
                                ...
                                console.log('We aborted the transaction (because ' + userId + '/' + groupId + '/' + POId + ' already exists).');
                            }
                            else {
                                console.log(userId + '/' + groupId + '/' + POId + ' added!'.green);
                            }
    ...
    deleteMessage: function(key) {
        ...
        //当 Event/E0001/100001/02_PU/PO_HEADERS/4500017496被删除
        //对应的删除user/100001/messages/E0001/02_PU/4500017496
