                    ...
                    snap.forEach(function(childsnap) {
                        ...
                        childsnap.child('PO_HEADERS')
                            .forEach(function(POHeader_childsnap) {
                                ...
                                var EventHistoryRefStr = POHRefStr.replace('Event', 'EventHistory');
                                var EventHistoryRef = fbutil.fbRef(EventHistoryRefStr);
                                var result;
                                // EventHistoryRef.once('value', function(snap) {
                                //     if (!snap.exists()) {
                                //         snap.ref().set(true);
                                        step1();
                                //     }
                                ...
                                function step1() {
                                    var metadata = {
                                        ...
                                        'vend_NAME': POHeader_childsnap.child('vend_NAME').val()
                                    };
                                    ...
                                    var po_ref = self.userRootRef.child(FBuserId)
                                        .child(self.path.user_message_field).child(self.event).child('header').child(POId);
                                    var po_item_ref = self.userRootRef.child(FBuserId)
                                        .child(self.path.user_message_field).child(self.event).child('items').child(POId); 
                                    po_ref.transaction(function(currentData) {
                                        if (currentData === null) {
                                            ...
                                            po_ref.child('data/release_group').once('value', function(nameSnapshot) {
                                                ...
                                                release_groupStr = release_groupStr + '||' + release_groupSubstr;
                                        ...
                                        .once('value', function(snap) {
                                            ...
                                            snap.forEach(function(childsnap) {
                                                ...
                                                var itemData = {
                                                    ...
                                                    'po_ITEM': childsnap.child('po_ITEM').val(),
                                                    'pur_MAT': childsnap.child('pur_MAT').val(),
                                                    'short_TEXT': childsnap.child('short_TEXT').val(),
                                                    'price_UNIT': childsnap.child('price_UNIT').val(),
                                                    'po_UNIT_ISO': childsnap.child('po_UNIT_ISO').val(),
                                                    'Ref':HeadData.data.ItemRef+'/'+itemID
                                                };
                                                if (n < 6) {
                                                    po_ref.child('items').child(itemID).set(itemData);
                                                }
                                                else {
                                                    po_item_ref.child('moreItems').child(itemID).set(itemData);
                                                }
                                            });
                                        });
                                }
